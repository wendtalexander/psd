...
def main() -> None:
    ...
    for model in models:
        basepath: Path = (
            find_project_root() / "data" / "punit" / "nperseg_fs" / model.cell
        )
        npersegs = [2**15, 2**17, 2**19, 2**20]
        fss = [20_000, 40_000, 80_000, 160_000]
        max_duration = 1.5
        for nperseg in npersegs:
            for fs in fss:
                savepath = basepath / f"nperseg-{nperseg}_fs-{fs}"
                ...
                for nix_file in nix_files:
                    if nix_file.is_file():
                        ...
                        nix_file.unlink()
                duration = jnp.ceil(max_duration * nperseg * (1 / fs))
                config = Config(
                    fs=fs,
                    duration=duration,
                    batch_size=25,
