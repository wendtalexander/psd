...
from psd.punit.punit_model_jax import simulate_without_adaptation
...
def simulation(config: Config, params: punit.PUnitParams):
    ...
    simulate = jax.vmap(jax.jit(simulate_without_adaptation), in_axes=[0, 0, None])
...
def main() -> None:
    ...
    for model in models:
        savepath: Path = (
            find_project_root() / "data" / "punit" / "without_adaptation" / model.cell
        )
