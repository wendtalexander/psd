...
def simulation(config: Config, params: punit.PUnitParams):
    ...
    simulate = jax.vmap(jax.jit(simulate_vdend), in_axes=[0, 0, None])
    ...
    for con, contrast in enumerate(config.contrasts):
        ...
        for batch in jnp.arange(0, config.trials, config.batch_size):
            ...
            spikes, v_dend = simulate(
                keys[1, con, batch : batch + config.batch_size, :], stimulus, params
            )
            rate = spike_rate(v_dend[:, -config.nperseg :], kernel)
            sm.update(v_dend, wh, rate)
        sm.norm()
...
def main() -> None:
    ...
    for model in models:
        savepath: Path = find_project_root() / "data" / "punit" / "vdend" / model.cell
